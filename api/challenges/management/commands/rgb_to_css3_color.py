...
class Command (BaseCommand):
    help = "Convert RGB hex value to nearest named CSS3 color."
    ...
    def add_arguments (this, parser):
        parser.add_argument ('rgb_hex', nargs = 1)

    def handle (this, *args, **kwargs):
        arg_rgb = kwargs ['rgb_hex'] [0]

        arg_r = int (arg_rgb [0:2], base = 16)
        arg_g = int (arg_rgb [2:4], base = 16)
        arg_b = int (arg_rgb [4:6], base = 16)

        color_distances = {}
        for name, css3_rgb in this.css3_colors.items ():
            css3_r = int (css3_rgb [0:2], base = 16)
            css3_g = int (css3_rgb [2:4], base = 16)
            css3_b = int (css3_rgb [4:6], base = 16)

            color_distances [(css3_r - arg_r) ** 2 + (css3_g - arg_g) ** 2 + (css3_b - arg_b) ** 2] = name

        print (color_distances [min (color_distances.keys ())])
